...
void YOLOv8_LT::drawDetected(
    ...
    for (int i = 0; i < num_results; ++i) {
        ...
        // Store the contours
        cropped_mask.convertTo(cropped_mask, CV_8UC1);
        std::vector<std::vector<cv::Point>> current_contours;
        cv::findContours(cropped_mask, current_contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);

        // Adjust the contour points to the original image coordinates
        for (auto& contour : current_contours) {
        ...
        if (show_bbox){
            // Draw bounding box
            cv::rectangle(result_image, cv::Point(x1, y1), cv::Point(x2, y2), random_color, 2);
        }
