...
int main() {
    ...
    try {
        ...
        cv::Mat image = cv::imread("/home/jh/Pictures/test/APD-0865_20240314_084916580_pave.jpg");
        cv::Mat input_image;
        ...
        std::cout << "mask_tensor shape: " << mask_tensor.sizes() << std::endl;

        // NMS
        auto keep = non_max_suppression(result_tensor)[0];
        // boxes(4)+conf(1)+classID(1)+MaskWeight(32)
        std::cout << keep.sizes() << std::endl;
